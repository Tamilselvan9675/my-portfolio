...
const FloatingDockDesktop = ({ items, className }) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "mx-auto hidden h-12 items-end gap-4 px-2 pb-1 md:flex",
        className,
      )}
    >
      {items.map(item => (
          ...
          <AnimatePresence>
            {hovered && (
              <motion.div
                ...
                className="absolute -top-6 left-1/2 w-fit rounded-md bg-white/10 border border-white/10 px-2 py-0.5 text-xs whitespace-pre text-white backdrop-blur-sm"
              >
                {title}
              </motion.div>
            )}
